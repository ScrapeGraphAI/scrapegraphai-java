...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ScrapegraphaiInvalidDataException("`$name` is not set")
            is JsonNull -> throw ScrapegraphaiInvalidDataException("`$name` is null")
            else -> throw ScrapegraphaiInvalidDataException("`$name` is invalid, received $this")
        }
